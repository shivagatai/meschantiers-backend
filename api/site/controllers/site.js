...
module.exports = {
        ...
        try {
          ...
          if (site.INSEE_NV) {
            ...
            if (!commune) {
              ctx.throw(
                400,
                "Impossible de trouver la commune associée : " + site.INSEE_NV
              );
            }
            ...
            console.log(
              "Impossible de charger " +
                site.NOM_CORRIGE_DBR +
                " : Pas de commune associée"
            );
          }
        } catch (err) {
  ...
  async loadcoordinates(ctx) {
    if (ctx.is("multipart")) {
      ...
      // finally loop through the json and fire the Strapi update queries
      await json.features.map(async (site) => {
        try {
          //        console.log(site)
          if (site.properties.identifiant_de_l_etablissement) {
            const etab = await strapi.services.site.findOne({
              code_uai_rattachement:
                site.properties.identifiant_de_l_etablissement,
            });
            if (!etab) {
              ctx.throw(
                400,
                "Impossible de trouver l’établissement associé : " +
                  site.properties.identifiant_de_l_etablissement +
                  " (" +
                  site.properties.nom_etablissement +
                  ")"
              );
            }
            if (!etab.coordinates) {
              const { longitude, latitude } = site.properties;
              const coordinates = { longitude, latitude };

              const entity = { coordinates: coordinates };
              await strapi.services.site.update({ id: etab.id }, entity);
            } else {
              //            console.log(                "Etablissement " +                  etab.nom_corrige_dbr +                  " a déjà des coordonnées"              );
            }
          } else {
            console.log("Impossible de charger le site " + site);
          }
        } catch (err) {
          console.log("Exception : ", err);
        }
